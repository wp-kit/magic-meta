...
```wp-kit/magic-meta``` relies on a ```::getMeta``` method on the Model to return the ```meta_value```, this is exactly what ```drewjbartlett/wordpress-eloquent``` provides, but in theory you can use any WordPress ```Eloquent``` provider that provides the same method and response.
